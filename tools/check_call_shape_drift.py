...
APP_MEMBER = re.compile(
    r"(?<![\w.])app\.(mount|controls|presentModal|installTheme|dispose|environment|core|present)\b"
)
...
def facet_members():
    ...
    match = re.search(r"return table\.freeze\(\{(.*?)\n\}\)", source, re.S)
    if match is None:
...
def selftest():
    controls = public_controls()
    members = facet_members()
    if "Button" not in controls or "VStack" in controls or "controls" not in members or "new" in members:
    ...
    plants = {
        ...
        "tests/call_shape_app_probe_tmp.luau": "app.mount(function() end)\nreturn nil\n",
        "examples/call_shape_wrapped_probe_tmp.luau": "local x = Facet.newTable(\n\tFacet,\n\tcore,\n\t{}\n)\nreturn x\n",
        "docs/call_shape_probe_tmp.md":
            "# Probe\n\n`UI.ZStack` in prose is not code.\n\n```luau\nlocal row = UI.HStack({})\n```\n",
        "tests/native_public_surface.spec.luau": "local row = UI.ZStack({})\n",
    }
    expected = {
        "examples/call_shape_probe_tmp.luau": ["examples/call_shape_probe_tmp.luau:2: `UI.VStack`"],
        "bench/call_shape_facet_probe_tmp.luau": ["bench/call_shape_facet_probe_tmp.luau:1: `Facet.new`"],
        "tests/call_shape_app_probe_tmp.luau": ["tests/call_shape_app_probe_tmp.luau:1: `app.mount`"],
        "examples/call_shape_wrapped_probe_tmp.luau": ["examples/call_shape_wrapped_probe_tmp.luau:1: flat builder"],
        "docs/call_shape_probe_tmp.md": ["docs/call_shape_probe_tmp.md:6: `UI.HStack`"],
        "tests/native_public_surface.spec.luau": ["tests/native_public_surface.spec.luau:1: `UI.ZStack`"],
    }
    ...
    if clean:
        ...
        print("\n".join(clean[:20]))
        return 1
    print("check_call_shape_drift: SELFTEST PASS - planted UI.VStack, Facet.new, app.mount, a wrapped "
