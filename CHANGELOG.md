...
- `Facet.civilDate` is the civil date calendar from `main`: arithmetic, the numeric words of a locale and fixed-offset instants. `Facet.recipes.arithmetic.parse` is the bounded arithmetic parser from `main` for a number field. The `Facet` type gives `civilDate` the type `any`, because the old Luau solver cannot check the whole `Facet` type with the civil date function types. The `CivilDateModule` type gives typed use.
- `UI.TextInput` has the field chrome from `main`: `label` (activation focuses the field, and the label is not a focus stop), `requiredMark` notation, one `hint` or `errorText` line with the error mark, `leading` and `trailing` Instances in the plate, `controlSize`, `appearance` and `corners`. It also takes `readOnly`, `selectOnFocus` (`none`, `all` or `end`, once for each focus session) and `visibleLines` for a multiline field. A field without these options keeps its TextBox root. **Breaking:** the constructor type returns `TextBox | Frame`, and the TextInput type no longer requires `onChange` for a read-only `value` (the control still refuses it at run time).
